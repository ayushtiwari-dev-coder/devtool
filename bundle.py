...
EXTENSIONS = ('.py','.js','.html','.css','.jsx')
# Folders to ignore so the file doesn't get cluttered
IGNORE_DIRS = {'__pycache__', '.git', '.vscode', 'venv','node_modules','public','frontend','helping_folders','vite.config.js','index.html','eslint.config.js'}
    ...
    with open(OUTPUT_FILE, 'w', encoding='utf-8') as master:
        ...
        for root, dirs, files in os.walk('.'):
            ...
            for filename in files:
                if filename.endswith(EXTENSIONS) and filename != 'bundle.py' and 'load_test.py':
